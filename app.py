...
from utils import save_datamatrix, ip_to_hex
import os
...
if selection == "Project Info":
    ...
    - IP addresses to DataMatrix codes
    
    Based on the **PowerScan PM9600** scanner manual.
    
    **IP → Hex Logic**:
    1. Split IP (e.g. `192.168.0.1`)
    2. Convert to hex: `C0A80001`
    3. Generate DataMatrix code
    """)
    ...
    if st.button("Convert IP to DataMatrix") and ip_input:
        try:
            ...
            st.write(f"Hex representation: `{hex_string}`")
            st.image(path, caption="DataMatrix Code")
            with open(path, "rb") as f:
                st.download_button(" Download PNG", f, file_name=filename)
